...
const baseLink = {
  ...
  category: 'ARTICLE',
  createdAt: new Date(),
  metadata: { title: 'Test', aiSummary: 'Summary', previewImage: null, dynamicData: {} },
  intent: { inferredAction: 'TO_READ', source: 'WEB_EXT', rawContext: 'context' },
};
...
describe('LinksService', () => {
  ...
  describe('create', () => {
    it('should create a link and return link_id', async () => {
      ...
      const result = await service.create('user-1', { url: 'https://example.com', source: 'WEB_EXT' });
    ...
    it('should push job to ingestion queue', async () => {
      mockPrisma.link.create.mockResolvedValue({ id: 'link-1' });
      mockQueue.addLinkIngestionJob.mockResolvedValue(undefined);
      await service.create('user-1', { url: 'https://example.com', source: 'WEB_EXT' });
      expect(mockQueue.addLinkIngestionJob).toHaveBeenCalledWith(expect.objectContaining({ linkId: 'link-1' }));
    });

    it('should include context_text in queue job if provided', async () => {
      mockPrisma.link.create.mockResolvedValue({ id: 'link-1' });
      mockQueue.addLinkIngestionJob.mockResolvedValue(undefined);
      await service.create('user-1', { url: 'https://example.com', source: 'WEB_EXT', context_text: 'hello' });
      expect(mockQueue.addLinkIngestionJob).toHaveBeenCalledWith(expect.objectContaining({ contextText: 'hello' }));
    });
  ...
  describe('updateStatus', () => {
    it('should update status successfully', async () => {
      mockPrisma.link.findUnique.mockResolvedValue({ id: 'link-1', userId: 'user-1' });
      mockPrisma.link.update.mockResolvedValue({ id: 'link-1', status: 'ARCHIVED' });
    ...
    it('should throw NotFoundException if link not found', async () => {
      ...
      await expect(service.updateStatus('bad-id', 'user-1', 'ARCHIVED')).rejects.toThrow(NotFoundException);
    });

    it('should throw ForbiddenException if userId mismatch', async () => {
      mockPrisma.link.findUnique.mockResolvedValue({ id: 'link-1', userId: 'other-user' });
      await expect(service.updateStatus('link-1', 'user-1', 'ARCHIVED')).rejects.toThrow(ForbiddenException);
    });
  });
  ...
  describe('reconstructContext', () => {
    ...
    it('should throw NotFoundException if link missing', async () => {
      mockPrisma.link.findUnique.mockResolvedValue(null);
      await expect(service.reconstructContext('bad', 'user-1')).rejects.toThrow(NotFoundException);
    });

    it('should throw ForbiddenException if wrong user', async () => {
      mockPrisma.link.findUnique.mockResolvedValue({ ...baseLink, userId: 'other' });
      await expect(service.reconstructContext('link-1', 'user-1')).rejects.toThrow(ForbiddenException);
    });

    it('should handle missing intent gracefully', async () => {
      mockPrisma.link.findUnique.mockResolvedValue({ ...baseLink, intent: null });
      const result = await service.reconstructContext('link-1', 'user-1');
  ...
  describe('markAsProcessed', () => {
    it('should update link to ACTIVE status', async () => {
      mockPrisma.link.update.mockResolvedValue({ ...baseLink, status: 'ACTIVE' });
      const result = await service.markAsProcessed('link-1', { title: 'Title', category: 'DEV' });
    ...
    it('should upsert metadata on process', async () => {
      ...
      await service.markAsProcessed('link-1', { title: 'T', aiSummary: 'S', previewImage: 'img.png' });
    ...
    it('should set inferredAction from data', async () => {
      mockPrisma.link.update.mockResolvedValue(baseLink);
      await service.markAsProcessed('link-1', { inferredAction: 'CODE_REVIEW' });
      const updateCall = mockPrisma.link.update.mock.calls[0][0];
