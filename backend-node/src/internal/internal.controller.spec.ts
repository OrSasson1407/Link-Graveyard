...
describe("InternalController", () => {
  ...
  it("should return 401 with wrong secret", async () => {
    await expect(
      controller.handleProcessedCallback(
        { link_id: "l1", user_id: "u1", success: true },
        "wrong-secret",
      ),
  ...
  it("should call updateStatus ERROR on failed scrape", async () => {
    await controller.handleProcessedCallback(
      { link_id: "l1", user_id: "u1", success: false },
      "test-secret",
    );
  ...
  it("should call markAsProcessed with AI result", async () => {
    ...
    expect(mockLinks.markAsProcessed).toHaveBeenCalledWith(
      "l1",
  ...
  it("should schedule reminder after successful processing", async () => {
    ...
    expect(mockQueue.addReminderSchedulerJob).toHaveBeenCalledWith({
      userId: "u1",
    });
  });
  ...
  it("should return ok:true on success", async () => {
    mockLinks.markAsProcessed.mockResolvedValue({});
    const result = await controller.handleProcessedCallback(
      { link_id: "l1", user_id: "u1", success: true },
      "test-secret",
    );
  ...
  it("should pass preview_image to markAsProcessed", async () => {
    mockLinks.markAsProcessed.mockResolvedValue({});
    await controller.handleProcessedCallback(
      { link_id: "l1", user_id: "u1", success: true, preview_image: "img.png" },
      "test-secret",
    );
    expect(mockLinks.markAsProcessed).toHaveBeenCalledWith(
      "l1",
      expect.objectContaining({ previewImage: "img.png" }),
    );
  });
  ...
  it("should pass title to markAsProcessed", async () => {
    mockLinks.markAsProcessed.mockResolvedValue({});
    await controller.handleProcessedCallback(
      { link_id: "l1", user_id: "u1", success: true, title: "My Title" },
      "test-secret",
    );
    expect(mockLinks.markAsProcessed).toHaveBeenCalledWith(
      "l1",
