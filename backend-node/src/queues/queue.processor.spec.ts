...
const mockQueue = {
  addDomScrapingJob: jest.fn(),
  addReminderSchedulerJob: jest.fn(),
};
const mockAi = {
...
describe("QueueProcessor", () => {
  ...
  describe("handleIngestion", () => {
    it("should forward valid URL to dom-scraping-queue", async () => {
      await processor.handleIngestion({
        data: { linkId: "l1", url: "https://x.com", userId: "u1" },
      } as any);
      expect(mockQueue.addDomScrapingJob).toHaveBeenCalledWith(
    ...
    it("should set link status to ERROR on invalid URL", async () => {
      await expect(
        processor.handleIngestion({
          data: { linkId: "l1", url: "not-a-url", userId: "u1" },
        } as any),
      ).rejects.toThrow();
  ...
  describe("handleAiAnalysis", () => {
    it("should call analyzeLink with correct payload", async () => {
      ...
      await processor.handleAiAnalysis({
        data: {
          linkId: "l1",
          url: "https://x.com",
          userId: "u1",
    ...
    it("should emit link.processed WebSocket event", async () => {
      ...
      expect(mockGateway.emitLinkProcessed).toHaveBeenCalledWith(
        "u1",
        "l1",
        expect.any(Object),
      );
